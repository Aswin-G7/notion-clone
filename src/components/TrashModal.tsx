    ...
    <div className="flex flex-col">
      <div
        ...
        className={`flex items-center justify-between py-2 pr-2.5 rounded-lg transition-colors group ${
          isDirectMatch
            ? "bg-amber-50/90 hover:bg-amber-100/80 border border-amber-200/80"
            : "hover:bg-stone-50"
        }`}
      >
        <div className="flex items-center gap-2 min-w-0 flex-1 pr-2">
          ...
          {hasChildren ? (
            <button
              ...
              className="p-0.5 text-stone-400 hover:text-stone-700 hover:bg-stone-200/60 rounded transition-colors cursor-pointer shrink-0"
            ...
            <div className="flex items-center gap-1.5">
              <span
                className={`text-xs font-semibold truncate ${
                  isDirectMatch ? "text-amber-950 font-bold" : "text-stone-800"
                }`}
              >
                {page.title.trim() === "" ? "Untitled Page" : page.title}
              </span>
              {isDirectMatch && (
                <span className="px-1.5 py-0.2 text-[9.5px] font-semibold bg-amber-200/80 text-amber-900 rounded shrink-0">
                  Match
                </span>
              )}
            </div>
            ...
            <div className="flex items-center gap-2 text-[10.5px] text-stone-400">
              ...
              {parentTitle && (
                <span className="flex items-center gap-1 truncate text-stone-400">
                  <span>•</span>
                  <FolderTree className="h-3 w-3 text-stone-300 shrink-0" />
                  <span className="truncate">In {parentTitle}</span>
                </span>
              )}
              {descendantCount > 0 && (
                <span className="flex items-center gap-1 text-stone-500 font-medium shrink-0">
                  <span>•</span>
                  <FolderTree className="h-3 w-3 text-stone-400 shrink-0" />
                  <span>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <button
            ...
            className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium text-stone-700 bg-white hover:bg-stone-100 border border-stone-200 rounded-md transition-all shadow-xs cursor-pointer"
...
export const TrashModal: React.FC<TrashModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-stone-900/40 backdrop-blur-sm animate-in fade-in duration-150">
      <div
        id="trash-modal-container"
        className="w-full max-w-xl bg-white border border-stone-200/90 shadow-2xl rounded-xl flex flex-col max-h-[85vh] overflow-hidden text-stone-800 font-sans animate-in zoom-in-95 duration-150"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-stone-100 bg-stone-50/60">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-stone-800 tracking-tight">Trash</h2>
            <span className="text-[11px] font-mono font-medium px-2 py-0.5 rounded-full bg-stone-200/70 text-stone-600">
          ...
          <div className="flex items-center gap-2">
            {trashPages.length > 0 && (
              <button
                ...
                className="px-2.5 py-1 text-xs font-semibold text-red-600 hover:text-red-700 hover:bg-red-50 border border-red-200/80 rounded-md transition-all cursor-pointer"
              >
                Empty Trash
              </button>
            )}
        ...
        {/* Footer info */}
        <div className="px-5 py-2.5 bg-stone-50/80 border-t border-stone-100 text-[11px] text-stone-400 flex items-center justify-between">
      ...
      {confirmDeleteTarget && (
        <div className="fixed inset-0 z-60 flex items-center justify-center p-4 bg-stone-900/50 backdrop-blur-xs animate-in fade-in duration-100">
          <div className="w-full max-w-md bg-white border border-stone-200 shadow-2xl rounded-xl p-5 text-stone-800 font-sans space-y-4 animate-in zoom-in-95 duration-100">
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-stone-100">
              <button
                ...
                className="px-3 py-1.5 text-xs font-semibold text-stone-600 hover:bg-stone-100 rounded-md transition-colors cursor-pointer"
              >
                Cancel
              </button>
              <button
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-stone-500 leading-relaxed">
                  ...
                  <strong className="text-stone-800">{trashPages.length}</strong> items in Trash? All pages and subpages will be removed forever. This action cannot be undone.
                </p>
              </div>
            </div>
